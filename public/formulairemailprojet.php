...
//On check les infos transmises lors de la validation
  ...
  header('Location: formulairepartenaireprojet.php');
  }else{
  $_SESSION['success'] = 1;
  $headers  = 'MIME-Version: 1.0' . "\r\n";
  $headers .= 'Content-type: text/html; charset=utf-8' . "\r\n";
  $headers .= 'FROM:' . htmlspecialchars($_POST['email']);
  $to = 'sarah.louette@outlook.fr'; // adresse email ICI
  $subject = 'Message envoyé pour le formulaire partenaire de projet par' . htmlspecialchars($_POST['name']) .' - <i>' . htmlspecialchars($_POST['email']) .'</i>';
  $message_content = '
  <table>
  <tr>
